...
###### Ajout des hauteurs de chute à partir d'une extraction provenant de BDOE ######
...
#Reprojection de des usra sur la BD TOPO pour obtenir les noms des CE
...
Info <-
  ...
  dplyr::select(-toponyme_c) %>%  sf::st_drop_geometry()
...
Info <-   
  ...
  dplyr::filter(Info, nom_CE_valid %in% ROE_Normandie_H$nom_CE_valid) 
  
#304/697 sans traitement approfondi
#Avec repojection sur bdtopo -> 469 /697
