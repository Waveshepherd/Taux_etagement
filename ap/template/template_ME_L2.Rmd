...
params:
  CME_L2: "FRHR299B"
  ME: "L'Orne du confluent de la  Baize (exclu) au confluent du Noireau (exclu)"
...
# [Caractéristiques de la masse d'eau]{.underline}
...
Info_ME_L2 <- ME_L2_lin %>%  dplyr::filter(cdeumassed == params$CME_L2) %>% dplyr::select((!nb_ouvp:H_cum))

L_Info_ME_L2 <- Info_ME_L2 %>% dplyr::select(!TE) %>% dplyr::mutate(dplyr::across(3:6, round, 0),
                                                    pt_nat = round(pt_nat,1)) %>% 
  knitr::kable(
    col.names = c(
      "Code de la masse d'eau",
      "Nom de la masse d'eau classée liste 2",
      "Longueur du linéaire de masse d'eau (en km)",
...
tab_L2 <- 
  ROE_Normandie_H_L2 %>% dplyr::filter(code_ME_valid == params$CME_L2) %>%  sf::st_drop_geometry()
...
tab_ouv <- 
  ...
  dplyr::mutate(nb_ouv = nrow(tab_L2),
                nb_ouvp = nrow(ouv_p),
                d_ouvp = round(nrow(ouv_p)/Info_ME_L2$longueur,1)) %>% 
...
# [Carte de la masse d'eau : **``r params$ME``**]{.underline}
...
tab_L2 <-
  ROE_Normandie_H_L2 %>% dplyr::filter(code_ME_valid == params$CME_L2) %>%
  sf::st_as_sf(crs = 4326)

ME_sf_NOR  <- ME_sf_NOR %>% 
  sf::st_as_sf(crs = 4326) %>% dplyr::filter(cdeumassed == params$CME_L2) 
...
# [Répartition des hauteurs de chute sur le cours principal du linéaire de masse d'eau]{.underline}
...
ch <- ouv_p %>% dplyr::filter(!is.na(hauteur)) %>% 
  ...
  ggplot2::labs(fill = "État :", subtitle = sprintf(params$ME)) +
...
Ce graphique présente le nombre d'ouvrages par intervalles de hauteur de chute sur le cours principal du linéaire de masse d'eau et en fonction de leur état.  
...
### La masse d'eau : "**``r params$ME``**" ayant pour code(s) : **``r params$CME_L2``** a un taux d'étagement de : [**``r Info_ME_L2$TE``**]{.underline} %.
