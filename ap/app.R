...
## 3.b. Onglet de la fiche résumé à l'échelle d'un CE ##
  ...
  tags$h1("Répartition des hauteurs de chute sur l'ensemble du cours d'eau"),
...
## 4.a. Création des objet réactifs ##
  ...
  tab_region <- reactive({
    tab_region <- 
      Info %>% dplyr::relocate("nb_ouvp","nb_ouv", .before =  "H_cum") %>% 
...
## 4.a. Création des tableaux de rendus ##
  ...
  output$tab_region <- DT::renderDataTable(tab_region(),
                                           ...
                                           options = list(
                                            ...
                                            buttons = list( 
                                              list(extend = 'csv',   filename =  paste("tx_etagement_de_la_region/", format(Sys.time(), '%Y_%m_%d'),".csv")),
                                              list(extend = 'excel', filename =  paste("tx_etagement_de_la_region/", format(Sys.time(), '%Y_%m_%d'),".docx"))))
) 
  ...
  output$tab_info <- renderTable({
    ...
    tab_info <-
      Info %>%  dplyr::filter(Info$nom_CE_valid == CE()) %>%
  ...
  output$map <- leaflet::renderLeaflet({
    ...
    pal <- leaflet::colorFactor(palette = c("Existant" = "#fde725", "Détruit partiellement" = "#21918c",
                                            "Détruit entièrement" ="#440154"),
                                levels = tab_geo$etat)
    ...
    map <- leaflet::leaflet(tab_geo) %>%
      ...
      leaflet::addLayersControl(
        ...
        overlayGroups = c("ROE du cours d'eau","Cours d'eau"),
